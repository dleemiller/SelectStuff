...
# -------------------------------------------
# Dictionary of existing managers keyed by DB path
# -------------------------------------------
_db_managers_by_path: dict[str, "SQLiteManager"] = {}
...
def initialize_database(config) -> "SQLiteManager":
    ...
    This ensures that multiple calls with the same path return
    the same manager (shared connection), allowing multiple apps
    to share a DB if needed.
    """
    db_path = config.database  # or wherever your config stores it
    with _lock:
...
class SQLiteManager:
    ...
    def __init__(self, db_path: str):
        ...
        logger.info(f"SQLiteManager instantiated for path: {db_path}")
